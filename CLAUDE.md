...
- [x] **P1-3**: URL 入力 → ダウンロード(進捗バー/サイズ/%)→ 保存(Downloads/DroidDLP)の最小 UI。
      ボトムナビで Download / PoToken を切替。複数画質の**フォーマット選択 UI 実装済み**(resolve → 一覧 → 選択 → DL)。
...
- [x] 設定画面(最大同時DL数 / 保存サブフォルダ / 動的カラー、SharedPreferences 永続化、`SettingsStore` + `DroidDlpApp`)+ 通知権限フロー。
- [x] release ビルドの R8/リソース縮小 + `proguard-rules.pro`(NewPipe/Rhino/jsoup/JS bridge の keep)。
      テスト用に debug 署名で installable(配布時は実 keystore へ)。`assembleRelease` 緑。
- [x] CI(GitHub Actions: Node テスト + ktlintCheck + testDebugUnitTest + assembleDebug)。`.github/workflows/ci.yml`
...
> - 2026-06-23 フォーマット選択 UI(resolve→一覧→選択→DL、サービスは DL 専用に簡素化)+ GitHub Actions CI を追加。
> - 2026-06-23 P2 R8: release で minify/shrink 有効化 + `proguard-rules.pro`(NewPipe/Rhino keep)。`assembleRelease` 緑(debug 署名)。
> - 2026-06-23 ダウンロードキュー: `DownloadProgressBus`→`DownloadQueueBus`(リスト)、サービスを ID 採番 + `Semaphore` で並行DL化、UI はキュー一覧(個別/全 Cancel)。
> - 2026-06-23 設定画面: `SettingsStore`(SharedPreferences)+ `DroidDlpApp`。最大同時数→Semaphore、保存サブフォルダ→MediaStore、動的カラー→テーマ に結線。Settings タブ追加。
