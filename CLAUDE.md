...
- [~] **P1-2**: `DirectUrlStreamExtractor`(直リンク)で end-to-end ダウンロード動作(JVM テスト付き)。
      **残**: YouTube 抽出 backend の選定と PoToken 受け渡し。**注意: NewPipeExtractor は GPLv3 →
      配布ビルド全体が GPLv3 になる**。seam の背後に後付け。
