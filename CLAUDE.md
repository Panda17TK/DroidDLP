...
- [ ] **P0-3 (E2E)**: 実機 WebView + 実ネットワークでの E2E(実トークン生成・YouTube 受理・Kotlin
      ブリッジの3引数 dispatch・仮想 youtube.com オリジンの妥当性)を instrumented で検証。
      ※本環境では検証不可(device-only)。最初の実機失敗候補は JNN の 403(UA/key/header で緩和済み)。
