...
└── app/
    ...
    └── src/
        ...
        │   │   └── ui/               # PoTokenScreen / PoTokenViewModel + theme/
...
- [ ] **P1-2**: 抽出 backend の選定と PoToken 受け渡し。**注意: NewPipeExtractor は GPLv3 →
      配布ビルド全体が GPLv3 になる**。当面 `DirectUrlStreamExtractor`(直リンク)で end-to-end 動作させ、
      YouTube backend は seam の背後に後付け。
- [ ] **P1-3**: URL 入力 → 解析 → フォーマット選択 → ダウンロード(進捗)の最小 UI(Compose)。
- [ ] **P1-4**: フォアグラウンドサービス + 通知(画面を閉じても継続)、POST_NOTIFICATIONS。
